...
export const TOOLS_CATALOG: ToolItem[] = [
  ...
  },
  {
    ...
    location: 'Sedimentation ➔ Secondary Tank Transfer Pipe',
  ...
  },
  {
    ...
    name: 'TDS Sensor Probe #2 (Post-RO)',
    category: 'SENSORS',
    model: 'DFRobot SEN0244 / Secondary Verifier',
    spec: '0–1000 ppm • Post-RO Permeate Quality Check',
    location: 'Tank 2 Post-Filtration Chamber (x = -1.85)',
    getLiveValue: (m) => `${m.tds2 || 28} ppm`,
    getStatus: (m) => (m.tds2 || 28) > 100 ? { label: 'RECIRCULATE', color: 'text-rose-400 bg-rose-950/60 border-rose-500/40' } : { label: 'POTABLE GRADE', color: 'text-emerald-400 bg-emerald-950/60 border-emerald-500/40' },
  },
  {
    id: 'ph2',
    preset: 'TANK2_VERIFICATION',
    name: 'pH Sensor Probe #2 (Post-RO)',
    category: 'SENSORS',
    model: 'E-201-C Glass Bulb Verifier',
    spec: '0–14.0 pH • Post-Mineralization Balance',
    location: 'Tank 2 Post-Filtration Chamber (x = -1.85)',
    getLiveValue: (m) => `${(m.ph2 || 7.35).toFixed(2)} pH`,
    getStatus: () => ({ label: 'BALANCED', color: 'text-emerald-400 bg-emerald-950/60 border-emerald-500/40' }),
  },
  {
    id: 'turbidity2',
    preset: 'TANK2_VERIFICATION',
    name: 'Turbidity Optical Sensor #2 (Post-RO)',
    category: 'SENSORS',
    model: 'TS-300B Nephelometric Verifier',
    spec: '0–100 NTU • 99.9% Clarity Validation',
    location: 'Tank 2 Post-Filtration Chamber (x = -1.85)',
    getLiveValue: (m) => `${(m.turbidity2 || 0.15).toFixed(2)} NTU`,
    getStatus: (m) => (m.turbidity2 || 0.15) > 1.0 ? { label: 'RECIRCULATING', color: 'text-amber-400 bg-amber-950/60 border-amber-500/40' } : { label: 'CRYSTAL PURE', color: 'text-emerald-400 bg-emerald-950/60 border-emerald-500/40' },
  },
  ...
  // ─── VESSELS & FILTERS ───
  {
    ...
    spec: '45 Liters • Coarse Gravel, Quartz Sand & Charcoal Beds',
    location: 'Raw Intake Side (x = 1.90, y = 0.05)',
    getLiveValue: () => '45 Liters Settling Capacity',
    getStatus: () => ({ label: 'GRIT TRAP ACTIVE', color: 'text-cyan-400 bg-cyan-950/60 border-cyan-500/40' }),
  },
  {
    id: 'tank2_verification',
    preset: 'TANK2_VERIFICATION',
    name: 'Quality Verification Chamber (Tank 2)',
    category: 'VESSELS',
    model: 'Post-Filtration Purity Gate Vessel',
    spec: '65 Liters • Multi-Sensor Verification & Automated Solenoid Recirculation',
    location: 'Verification Chamber (x = -1.85, y = 0.15)',
    getLiveValue: (m) => m.tds > 100 ? `Gate: IMPURE (${m.tds} ppm)` : `Gate: PURE (${m.tds || 28} ppm)`,
    getStatus: (m) => m.tds > 100 
      ? { label: 'RECIRCULATING TO STAGE 1', color: 'text-amber-400 bg-amber-950/60 border-amber-500/40' } 
      : { label: 'VERIFIED PURE ➔ STORAGE', color: 'text-emerald-400 bg-emerald-950/60 border-emerald-500/40' },
  },
  {
    ...
    category: 'VESSELS',
    model: '3-Way Solenoid & Stainless Return Riser',
    spec: 'Diverts Sub-Standard Water Directly into Stage 1 SediShield',
    location: 'Return Conduit (x = -1.35, y = 0.05)',
    getLiveValue: (m) => m.tds > 100 ? 'Return Active: 3.8 L/min' : 'Return Standby (0 L/min)',
    getStatus: (m) => m.tds > 100 
      ? { label: 'RE-FILTERING (CLOSED-LOOP)', color: 'text-rose-400 bg-rose-950/60 border-rose-500/40' } 
      : { label: 'STANDBY (SOLENOID CLOSED)', color: 'text-zinc-400 bg-zinc-900 border-zinc-700' },
  },
  {
    id: 'filter_housing',
    preset: 'FILTER_HOUSING',
    name: '4-Stage Smart Filtration Train',
    category: 'VESSELS',
    model: 'SediShield + ChemoBlock + RO Maxx + Active Copper Filter',
    spec: 'Borewell & Mining Defense • Particles, Chemicals, Salts, Copper Mineral Infusion',
    location: 'Back-Mounted Filtration Rack (x = -0.70, z = -0.62)',
    getLiveValue: (m) => `Health: 91% (Sedi: ${m.stage1Health || 92}%, Chemo: ${m.stage2Health || 85}%, RO: ${m.stage3Health || 88}%, Copper: ${m.stage4Health || 95}%)`,
    getStatus: (m, mode) => mode === 'TURBIDITY' ? { label: '4-STAGE PURIFYING', color: 'text-amber-400 bg-amber-950/60 border-amber-500/40' } : { label: '4-STAGE DEFENSE ACTIVE', color: 'text-cyan-400 bg-cyan-950/60 border-cyan-500/40' },
  },
  {
    id: 'secondary_tank',
